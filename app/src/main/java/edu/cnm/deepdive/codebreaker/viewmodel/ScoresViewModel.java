...
public class ScoresViewModel extends AndroidViewModel {
  ...
  private static class ScoreboardFilterLiveData extends MediatorLiveData<ScoreboardParams> {
    ...
    public ScoreboardFilterLiveData(
      ...
      addSource(codeLength, (length) -> setValue(new ScoreboardParams(length, poolSize.getValue(),
          sortedByTime.getValue())));
      addSource(poolSize, (size) -> setValue(new ScoreboardParams(codeLength.getValue(), size, sortedByTime.getValue())));
      addSource(sortedByTime, (sorted) -> setValue(new ScoreboardParams(codeLength.getValue(), poolSize.getValue(), sorted)));
    }
  }
